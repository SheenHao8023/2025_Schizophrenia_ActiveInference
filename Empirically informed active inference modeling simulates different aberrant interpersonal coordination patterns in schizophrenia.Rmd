  ...
  for (tvar in target_vars) {
    ...
    for (i in 1:length(all_results)) {
      ...
      if (!is.na(result$r) && result$n > 2) {
        cat(sprintf(
          "%-25s r = %6.3f, 95%% CI = [%.3f, %.3f], n = %2d, p_raw = %.4f, p_fdr = %.4f, p_bonf = %.4f\n",
          result$variable, result$r, result$ci_low, result$ci_high,
          result$n, result$p_raw, p_fdr[i], p_bonf[i]
        ))
      } else {
        cat(sprintf("%-25s data default\n", result$variable))
      }
    }
  }
  cat("\n")
