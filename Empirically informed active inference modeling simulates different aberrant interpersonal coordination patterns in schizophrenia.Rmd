...
plot_data_sig <- plot_data %>%
  filter(!is.na(ci_low) & ci_low * ci_high > 0)   
...
if (nrow(plot_data_sig) > 0) {
  ...
  p <- ggplot(plot_data_sig,aes(x = combo, y = r, color = Group)) +
    ...
    geom_rect(aes(xmin = 0.7, xmax = 3.3, ymin = -0.05, ymax = 0),fill = "grey", alpha = 1, color = NA)+
    geom_rect(aes(xmin = 3.7, xmax = 5.3, ymin = -0.05, ymax = 0),fill = "grey", alpha = 1, color = NA)+
    geom_rect(aes(xmin = 5.7, xmax = 8.3, ymin = -0.05, ymax = 0),fill = "grey", alpha = 1, color = NA)
