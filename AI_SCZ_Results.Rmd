...
chi_square_perm_test <- function(data, var, group_var = "Group2", B = 10000) {
...
print_chi_result <- function(res) {
  ...
  cat(sprintf("\n卡方检验: χ²(%.0f) = %.2f, p = %.4f", res$df, res$chi_square, res$p_value))
...
wilcoxon_perm_test <- function(df, var1, var2, B = 10000) {
...
kruskal_perm_test <- function(data, var, group_var = "Group2", B = 10000) {
  ...
  if (sig && length(res$groups) >= 2) {
    ...
    for (i in seq_along(pair_results)) {
      pair_results[[i]]$p_raw  <- raw_ps[i]
      pair_results[[i]]$p_fdr  <- adj_ps_fdr[i]
      pair_results[[i]]$p_bonf <- adj_ps_bonf[i]
      pair_results[[i]]$sig <- adj_ps_bonf[i] < 0.05  # 默认显著性依据Bonf
    }
...
print_kw_result <- function(res) {
  ...
  cat("变量:", res$var, "\n")
  cat(rep("-", 50), "\n", sep = "")
  cat(sprintf("Kruskal-Wallis检验: H = %.2f, p = %.4f, eta² = %.3f", 
...
## 三组之间的统计检验
```{r stat-posthoc}
variables <- c("Age", "IRI_Fantasy", "IRI_EmpathicConcern", "IRI_Perspective",  
...
hiddenstateprobs_sub <- hiddenstateprobs %>%
  ...
  mutate(Group2 = factor(group, levels = c("HC", "PS", "NS")))
...
for (st in states) {
  ...
  pairwise_df <- pairwise_raw %>%
    ...
    left_join(pairwise_bonf %>%
                ...
                rename(p_bonferroni = p.value),
              by = "contrast") %>%
    mutate(across(c(estimate, SE, df, t.ratio, p_uncorrected, p_bonferroni),
                  ~ round(.x, 4)))
  
  # 样本量
  n_per_group <- table(dat_sub$Group2)
...
for (col in cols_to_test) {
  ...
  for (grp in unique(data$Group)) {
    ...
    if (length(fit_list) > 0) {
      ...
      best_aic <- names(which.min(comp$aic))
      ...
      p10 <- quantile(subset_data, 0.10, na.rm = TRUE)
      p90 <- quantile(subset_data, 0.90, na.rm = TRUE)
      # ---- 偏度指标 ----
      skew_q <- as.numeric((q75 + q25 - 2 * q50) / (q75 - q25))   # 中部偏度
      skew_tail <- as.numeric((p90 + p10 - 2 * q50) / (p90 - p10)) # 尾部偏度
      # ---- 打印结果 ----
      cat("\nGroup:", grp, "\n")
      cat("  AIC最优分布:", best_aic, "\n")
      cat("  BIC最优分布:", best_bic, "\n")
      cat("  中部分位数: 25% =", round(q25, 4),
          "  中位数 =", round(q50, 4),
          "  75% =", round(q75, 4), "\n")
      cat("  尾部分位数: 10% =", round(p10, 4),
          "  90% =", round(p90, 4), "\n")
      cat("  Bowley Skewness (中部):", round(skew_q, 3),
          " | Tail Skewness (尾部):", round(skew_tail, 3), "\n")
      ...
      if (abs(skew_tail) < 0.1) {
        cat("；尾部近似对称\n")
      } else if (skew_tail > 0.1) {
        cat("；尾部右尾较长\n")
      } else {
        cat("；尾部左尾较长\n")
      }
...
generate_scatter_plots <- function(data, group_colors, group_name, outcome_matrix, save_path) {
  for (predictor in vars) {
    ...
    for (group in unique(data$Group)) {
      ...
      model_beta <- lm.beta(model)  # 计算标准化回归系数
      slope <- coef(model_beta)[2]  # 标准化 beta
